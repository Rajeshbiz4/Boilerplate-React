...
export const getCookie = (key) =>
  Rcookie.load(`${process.env.app}_${key}_${process.env.type}`);
  ...
  if (iStr) {
    ...
    for (let i = 0; i < _wordArray.length; i += 1) {
      if (_wordArray[i][0] === "@") {
        if (isRemove) {
          _wordArray[i] =
            _wordArray[i].indexOf("::") !== -1
              ? _wordArray[i].substr(0, _wordArray[i].indexOf("::"))
              : _wordArray[i];
          ...
          if (dictionary[_wordArray[i]].author_id) {
            _wordArray[i] = `${dictionary[_wordArray[i]]["mod-name"]}::${
              dictionary[_wordArray[i]].author_id
            }`;
          } else {
            _wordArray[i] = dictionary[_wordArray[i]]["mod-name"];
          }
